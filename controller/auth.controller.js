...
async function registerUser(req, res) {
  try {
    ...
    const newUser = new Users({ name, surname, email, username, password: hashedPassword, role: role || "user"});
...
async function userLogin(req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ username, accessToken, refreshToken, role: x });
  } catch (error) {
    console.log(error);
...
async function logoutUser(req, res) {
  ...
  return res.status(200).send({ message: "Log Out Successfull" });
}
...
module.exports = { registerUser, userLogin, logoutUser, viewAccount, updateAccount, verifyRefreshToken, isAdmin };